...
anes <- read.dta('../data/preparation/anes_timeseries_2012_stata12.dta')
...
# Drop obs with missingness on dependent var
df <- df[-which(is.na(df$libcpre_self)),  ]

write.table(df, '../data/preparation/anes_clean.csv', row.names = F,sep = ",")
